...
def patch_refiner() -> None:
    ...
    anchor = '''fn path_curve(path: &PathAllocation, market: &MarketState) -> Option<ContinuousCpmm> {
    let mut curve = None;
...
'''
    text = replace_once(text, anchor, replacement, "refiner path-curve patch")

    anchor2 = '''/// Sound upper bound on the exact output of a two-path V2 split over x in [0,T].
...
fn certified_v2_pair_upper(
'''
    text = replace_once(text, anchor2, helper, "refiner structural helper patch")
...
def patch_search() -> None:
    ...
    text = replace_once(
        ...
        'exact_v2_refiner::{certified_v2_interval_upper_for_paths, refine_disjoint_allocations},',
        "search import patch",
    )

    anchor = '''fn concavity_audit_enabled() -> bool {
    std::env::var_os("FYND_CONCAVITY_AUDIT").is_some()
}
'''
    replacement = '''fn concavity_audit_enabled() -> bool {
    std::env::var_os("FYND_CONCAVITY_AUDIT").is_some()
}
...
'''
    text = replace_once(text, anchor, replacement, "search env patch")

    marker = '''fn discover_paths(
...
'''
    text = replace_once(text, marker, census_code, "search census insertion")

    old_collect = '''    let collect_census =
        census_enabled() || symbolic_cut_census_enabled() || concavity_audit_enabled();
'''
    new_collect = '''    let collect_census = census_enabled()
        || symbolic_cut_census_enabled()
        || concavity_audit_enabled()
        || certified_interval_census_enabled();
'''
    text = replace_once(text, old_collect, new_collect, "search collect-census patch")

    old_call = '''    if concavity_audit_enabled() {
        super::concavity_audit::emit(&search.census_paths, total, ctx);
    }

    search.paths.sort_unstable_by(|a, b| b.amount_out.cmp(&a.amount_out));
'''
    new_call = '''    if concavity_audit_enabled() {
        super::concavity_audit::emit(&search.census_paths, total, ctx);
    }
    ...
    search.paths.sort_unstable_by(|a, b| b.amount_out.cmp(&a.amount_out));
'''
    text = replace_once(text, old_call, new_call, "search census call patch")
