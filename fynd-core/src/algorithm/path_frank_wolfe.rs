...
impl PathFrankWolfeAlgorithm {
        ...
        if self.exact_v2_refinement {
                ...
                refine_disjoint_allocations(&allocations, total_amount, &ctx.market_data)?
            {
                ...
                if refined_route.validate().is_ok() &&
                    Self::compute_split_net_amount_out(&refined_route, ctx)? >=
                        Self::compute_split_net_amount_out(&native_route, ctx)?
                {
                    allocations = refined;
                }
            }
    ...
    /// Searches pool-disjoint V2/V3 paths independently of Frank-Wolfe's committed candidate set.
    fn search_exact_disjoint(
        &self,
        ctx: &BellmanFordContext,
        order: &Order,
        floor: &RouteResult,
            ...
            search_disjoint_portfolios(ctx, order.amount(), self.max_hops, self.config.max_paths)?
        {
            ...
            let route = build_split_route(&allocations, &ctx.market_data, order)?;
            if route.validate().is_err() {
                continue;
            }
            let net = Self::compute_split_net_amount_out(&route, ctx)?;
            if net <= best_net {
                continue;
            }
            best_net = net.clone();
            let gas_price = ctx
                ...
                .clone()
                .unwrap_or_default();
            let mut result = RouteResult::new(route, net, gas_price);
            if let Ok(price_impact) = Self::compute_average_price_impact(&allocations) {
                result = result.with_price_impact(price_impact);
            }
            best = Some(result);
        }
        Ok(best)
...
impl Algorithm for PathFrankWolfeAlgorithm {
    ...
    async fn find_best_route(
        ...
        // Step 3: establish native Path Frank-Wolfe's result as the quality floor.
        let native = match split_result {
        ...
        if self.exact_v2_refinement {
            match self.search_exact_disjoint(&ctx, order, &native, start) {
                ...
                Ok(None) => {}
                Err(error) => debug!(?error, "exact V2 search failed, retaining native result"),
            }
        }
        Ok(native)
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn compare_pinned_uniswap_v2_snapshot() {
        let dai = token_with_decimals(0x01, "DAI", 18);
        let usdc = token_with_decimals(0x02, "USDC", 6);
        let weth = token_with_decimals(0x03, "WETH", 18);
        let usdt = token_with_decimals(0x04, "USDT", 6);
        let v2 = |reserve0: &str, reserve1: &str| -> Box<dyn ProtocolSim> {
            Box::new(UniswapV2State::new(
                U256::from_str_radix(reserve0, 10).unwrap(),
                U256::from_str_radix(reserve1, 10).unwrap(),
            ))
        };
        let (market, graph_manager) = setup_market_unweighted(vec![
            ("USDC/WETH", &usdc, &weth, v2("9969783373307", "4126437746588357099157")),
            ("DAI/WETH", &dai, &weth, v2("4502502058327543061982069", "1863580281953754627597")),
            ("WETH/USDT", &weth, &usdt, v2("2968398802599216703310", "7174216569646")),
            ("DAI/USDC", &dai, &usdc, v2("544146020174839211802609", "544868831773")),
            ("USDC/USDT", &usdc, &usdt, v2("1665372566665", "1668654631383")),
            ("DAI/USDT", &dai, &usdt, v2("17224634952356522844618", "17202855873")),
        ]);
        let algo = PathFrankWolfeAlgorithm::new(
            AlgorithmConfig::new(1, 4, StdDuration::from_secs(5), None).unwrap(),
            PathFrankWolfeConfig::default(),
        );
        let ord = order(&weth, &usdc, 1_000_000_000_000_000_000, OrderSide::Sell);
        let result = algo
            .find_best_route(graph_manager.graph(), market, None, None, &ord)
            .await
            .unwrap();
        ...
        let gross_out = route
            .swaps()
            .iter()
            .filter(|swap| swap.token_out() == &usdc.address)
            .map(|swap| swap.amount_out())
            .cloned()
            .sum::<BigUint>();
        assert_eq!(gross_out, BigUint::from(2_408_244_983u64));
        println!("Fynd gross output: {gross_out}");
    ...
    #[tokio::test]
    async fn exact_v2_refinement_splits_parallel_pools() {
        let token_a = token_with_decimals(0x01, "A", 18);
        let token_b = token_with_decimals(0x02, "B", 18);
        let reserve = U256::from_str_radix("100000000000000000000000", 10).unwrap();
        let v2 = || -> Box<dyn ProtocolSim> { Box::new(UniswapV2State::new(reserve, reserve)) };
        let (market, graph_manager) = setup_market_unweighted(vec![
            ("P1", &token_a, &token_b, v2()),
            ("P2", &token_a, &token_b, v2()),
        ]);
        let algo = pfw_algo_with_config(
            2,
            PathFrankWolfeConfig {
        ...
        let derived = derived_with_token_prices(&[&token_a, &token_b]);
        let amount = BigUint::from(50_000u64) * BigUint::from(10u64).pow(18);
        let ord = order(&token_a, &token_b, amount.to_u128().unwrap(), OrderSide::Sell);
        ...
        let difference = if swaps[0].amount_in() >= swaps[1].amount_in() {
            swaps[0].amount_in() - swaps[1].amount_in()
        } else {
            swaps[1].amount_in() - swaps[0].amount_in()
        };
        assert!(
            difference <= BigUint::from(1_000_000_000u64),
            "expected near-equal KKT allocation, got {} and {}",
            swaps[0].amount_in(),
            swaps[1].amount_in()
        );
        assert_eq!(
        ...
        let reserve = BigUint::from(100_000u64) * BigUint::from(10u64).pow(18);
        let quote = |input: &BigUint| {
            let input_with_fee = input * BigUint::from(9_970u32);
            &input_with_fee * &reserve / (&reserve * BigUint::from(10_000u32) + input_with_fee)
        };
        for swap in swaps {
            assert_eq!(swap.amount_out(), &quote(swap.amount_in()));
        }
        let split_output = swaps
            .iter()
            .map(|swap| swap.amount_out())
            .sum::<BigUint>();
        assert!(split_output > quote(&amount));
    }
}
