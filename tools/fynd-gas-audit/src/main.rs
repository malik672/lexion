...
struct Cli {
    ...
    /// Delay between receiving a quote and simulating it, to measure stale-state failures.
    ...
    for (i, trade) in sampled.iter().enumerate() {
        ...
        let (status, gas_estimate, actual_gas, error_reason) = match quoter.quote(trade).await {
            ...
            QuoteOutcome::Ok(quote) => {
                ...
                if cli.quote_delay_ms != 0 {
                    tokio::time::sleep(Duration::from_millis(cli.quote_delay_ms)).await;
                }
